...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 onRowClick:{},
			 singleClick:false,
			 maskAjaxRequest:false,
			 maskAjaxRequestMsg:'ajaxing...',
			 singleSelect:true
		  }, p);
		...
		var g = {
					 ...
					 function(i,row) 
					 	{
							...
							$(tr).attr('alt',row.id);
							if(p.singleClick)
								$(tr).click(p.onRowClick);
							else $(tr).dblclick(p.onRowClick);	
							//$(tr).bind('click',p.onRowSingleClick);
			...
			populate: function () { //get latest data
				...
				if (p.params)
					...
					if(p.maskAjaxRequest)
						$('BODY').mask(p.maskAjaxRequestMsg);
					$.ajax({
					   type: p.method,
					   url: p.url,
					   data: param,
					   ...
					   error: function(XMLHttpRequest, textStatus, errorThrown) { try { if (p.onError) p.onError(XMLHttpRequest, textStatus, errorThrown); } catch (e) {} $('BODY').unmask();}
					 });
			},
			doSearch: function () {
